...
namespace PruebaPDF.Controllers
{
    ...
    public class EditorHtmlController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> ModificarHtmlConVariasImagenesAPdf(IFormFile archivoHtml, List<IFormFile> imagenes, string NroEnvio)
        {
            if (archivoHtml == null || archivoHtml.Length == 0)
                return BadRequest("Debe proporcionar un archivo HTML válido.");
            if (imagenes == null || imagenes.Count == 0)
            ...
            var doc = new DinkToPdf.HtmlToPdfDocument()
            {
                GlobalSettings = new DinkToPdf.GlobalSettings
                {
                    ...
                    Margins = new DinkToPdf.MarginSettings { Top = 10 }
                },
                Objects = {
                    new DinkToPdf.ObjectSettings
                    {
                        HtmlContent = htmlContent,
                        WebSettings = new DinkToPdf.WebSettings { DefaultEncoding = "utf-8" }
                    }
                }
            };

            byte[] pdf = _converter.Convert(doc);
            return File(pdf, "application/pdf", "ArchivoModificado.pdf");
        }
